...
def import_data(fw,
    ...
    for row in range(nrows):
        ...
        try:
            ...
            try:
                lookup_string = f"{group_name}/{project_name}/{subject_label}/{session_label}"
                log.info(f"Checking for location {lookup_string}")
                ses = fw.lookup(lookup_string)
            except ApiException:
                log.error(f'No session found for: {lookup_string}\n please double check.  Skipping ')
                continue
            
            get_files = True
            objects_for_processing = fu.get_objects_for_processing(fw, ses, 'acquisition', get_files)
